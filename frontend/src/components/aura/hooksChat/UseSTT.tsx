...
<<<<<<< HEAD
  const batchRef = useRef<{
    ...
    batchBuffer: ArrayBuffer | null;
    batchView: DataView | null;
    batchInt16: Int16Array | null;
    batchOffset: number; // samples, not bytes
  }>({
    bufferPool: [],
    batchBuffer: null,
    batchView: null,
  ...
  // let batchView: DataView;
  // let batchInt16: Int16Array;
  // let batchOffset = 0;
  ...
  const initBatch = useCallback(() => {
    const s = batchRef.current;
    if (!s.batchBuffer) {
      s.batchBuffer = s.bufferPool.pop() || new ArrayBuffer(MESSAGE_BYTES);
      s.batchView   = new DataView(s.batchBuffer);
      s.batchInt16  = new Int16Array(s.batchBuffer, HEADER_BYTES);
      s.batchOffset = 0;
    }
  }, []);
  ...
  const flushBatch = useCallback(() => {
    const s = batchRef.current;
    if (!s.batchBuffer || !s.batchView) return;

    const ws = socketRef.current;
    if (!ws || ws.readyState !== WebSocket.OPEN) {
      s.bufferPool.push(s.batchBuffer);
      s.batchBuffer = null;
      s.batchInt16 = null;
      s.batchView = null;
      s.batchOffset = 0;
    ...
    s.batchView.setUint32(0, ts, false);
    s.batchView.setUint32(4, flags, false);
=======
...
<<<<<<< HEAD
  ...
  const flushRemainder = useCallback(() => {
    const s = batchRef.current;
    if (s.batchOffset > 0 && s.batchInt16) {
      for (let i = s.batchOffset; i < BATCH_SAMPLES; i++) s.batchInt16[i] = 0;
      flushBatch();
    }
  }, [flushBatch]);
...
<<<<<<< HEAD
      ...
      while (read < int16.length) {
        initBatch();
        const s = batchRef.current;
        const toCopy = Math.min(int16.length - read, BATCH_SAMPLES - s.batchOffset);
        s.batchInt16!.set(int16.subarray(read, read + toCopy), s.batchOffset);
        s.batchOffset += toCopy;
        read += toCopy;
        if (s.batchOffset === BATCH_SAMPLES) flushBatch();
      }
=======
