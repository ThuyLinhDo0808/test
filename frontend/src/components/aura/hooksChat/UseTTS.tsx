...
function prepareTimelineFromWords() {
  ...
  //console.log(wordQueueRef.current)
...
export function onPlaybackStarted(headStartSec?: number) {
  const ctx = audioCtxRef.current!;
  const lead = headStartSec ?? outputLatencySec;
  t0Ref.current = ctx.currentTime + lead;
  prepareTimelineFromWords();
  return t0Ref.current!
}
