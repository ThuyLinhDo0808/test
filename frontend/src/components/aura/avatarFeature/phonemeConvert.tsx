...
<<<<<<< HEAD
const VISEME_VOWELS = new Set<string>(Object.values(V_MAP));
=======
>>>>>>> a3e73f24fdecda8ff20136cc16c840bbb297c079
...
function decimateTimeline(events: VisemeEvent[]): VisemeEvent[] {
  ...
  const prefer = (e: VisemeEvent) => {
    const t = e.name;
<<<<<<< HEAD
    return (VISEME_VOWELS.has(t) ? 2 : 0) + (t === "viseme_CH" ? 1.5 : 0) + (t === "viseme_PP" ? 1 : 0);
=======
...
<<<<<<< HEAD
  ...
  //console.log("Adjusted words:", adjusted);

  // 2a) If already playing, append immediately
  if (t0Ref.current != null) {
    //console.log(`Word adjusted: ${adjusted}`); // try to log the word and its adjusted in  this 
