...
<<<<<<< HEAD
# ===================== Head pose mapping =====================
HEAD_YAW_LOG_K = 35.0     # sensitivity for yaw_ratio -> degrees (tune 25–45)
HEAD_PITCH_K   = 120.0    # deg per unit openness delta (tune 80–150)
PITCH_REF_OPEN = 0.30     # baseline openness (avg of eyes) when facing forward & neutral
HEAD_YAW_MAX   = 45.0     # clamp degrees
HEAD_PITCH_MAX = 30.0
HEAD_ROLL_MAX  = 30.0
...
def eye_metrics(lmk, w, h, inner_idx, outer_idx, top_idx, bot_idx, iris_idxs):
    ...
    width_px = abs(ix - ox)
<<<<<<< HEAD
    open_norm = abs(by - ty) / (width_px + 1e-6)  # eyelid openness normalized by width
=======
>>>>>>> a3e73f24fdecda8ff20136cc16c840bbb297c079
    return {
        "hx": float(hx),
...
<<<<<<< HEAD
        ...
        "corners": (ix, iy, ox, oy),  # useful for roll
=======
>>>>>>> a3e73f24fdecda8ff20136cc16c840bbb297c079
    }
...
<<<<<<< HEAD
        ...
        # ---- Distance gate (set your own numbers) ----
        self.NEAR_ON_CM  = 40.0   # start recognizing at ≤ 40 cm
        self.NEAR_OFF_CM = 45.0   # stop recognizing when > 45 cm
...
<<<<<<< HEAD
        ...
        dist = out["distance_cm"]
        if dist is not None:
            # Update gate with hysteresis
            if not self._gate_active and dist <= self.NEAR_ON_CM:
                self._gate_active = True
            elif self._gate_active and dist > self.NEAR_OFF_CM:
                self._gate_active = False

        # If gate is OFF, pretend there's no face for app logic
        if not self._gate_active:
            # Keep proximity/distance for UI, but suppress recognition
            out.update({
                "has_face": False,
                "looking": False,
                "gaze_dir": "—",
...
<<<<<<< HEAD
        # Map ratio to degrees: ratio=1 -> 0°, >1 -> +yaw (turn right), <1 -> -yaw
        head_yaw_deg = HEAD_YAW_LOG_K * math.log(yaw_ratio)
        head_yaw_deg = max(-HEAD_YAW_MAX, min(HEAD_YAW_MAX, head_yaw_deg))

        # ----- Head roll from eye-corner line slope -----
        r_ix, r_iy, r_ox, r_oy = m_right["corners"][0], m_right["corners"][1], m_right["corners"][2], m_right["corners"][3]
        l_ix, l_iy, l_ox, l_oy = m_left["corners"][0],  m_left["corners"][1],  m_left["corners"][2],  m_left["corners"][3]

        # Use outer corners for stability
        rx, ry = r_ox, r_oy
        lx, ly = l_ox, l_oy

        roll_rad = math.atan2((ly - ry), (lx - rx))  # screen y-down
        head_roll_deg = -math.degrees(roll_rad)      # right-ear-down -> positive
        head_roll_deg = max(-HEAD_ROLL_MAX, min(HEAD_ROLL_MAX, head_roll_deg))

        # ----- Head pitch from eyelid openness heuristic -----
        open_avg = 0.5 * (m_left["open_norm"] + m_right["open_norm"])
        # Look down / tuck chin -> smaller openness; look up -> larger openness
        head_pitch_deg = HEAD_PITCH_K * (PITCH_REF_OPEN - open_avg)
        head_pitch_deg = max(-HEAD_PITCH_MAX, min(HEAD_PITCH_MAX, head_pitch_deg))
